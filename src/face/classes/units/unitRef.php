...
class unitRef extends unitUnit
{
  ...
  function Handle() {
    ...
    if ($this->guide->uri_parts[1]=='create')    {
      $this->tpl->Set('CurrOperation', 'Новый сеанс');
    ...
    if ($this->guide->uri_parts[1]=='drop' && is_numeric($id=$this->guide->uri_parts[2]))    {
      if ($this->refList_drop($id)); {
        header ('Location: http://'.$_SERVER['HTTP_HOST']."/".$this->guide->proj_root.$this->guide->uri_parts[0].'/');
        die();
      }
    }
  ...
  function &getPlaces($places_data) {     // $places_data[0] = ' --------- ';
    ...
    if (!empty($pla)) $places_data[$pla[0]['id']] = $pla[0]['place_name'];
    else {
        $this->tbl->Init($this->guide->db_pfx.'place');
