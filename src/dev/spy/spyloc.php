...
foreach ( $cfd as $idx => $str ) {
    ...
    if ( strpos(' '.$str, $brand)) {
        $_mr = array();
        $_m  = explode('~',$str);
        foreach($_m as $mk=>$mv) $_mr[$mk]=trim($mv); // ,"\r\n\t"); // $_m[9] == 'shure'
...
// echo '<hr /> new CFD : '.var_export($new_cfd,1);
$dbf = implode( "", $new_cfd );

$new_dbf = 'tov_db.'.date("Ymd.His").'.txt';
$fw = fopen($new_dbf, 'w') or die('cannot create/open <a href="'.$new_dbf.'">'.$new_dbf.'</a> for writing');
$wr = fwrite ($fw, $dbf);
echo 'writing DBF : '.$wr.'<br />';
echo 'DBF: <a href="'.$new_dbf.'">'.$new_dbf.'</a> <br />';
