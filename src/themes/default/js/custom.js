...
function elSwitchAdv ( node, defview ) { // for cross-browser restoring view of TR elements use defview = ''
  if (defview==undefined) defview = 'block';
  var el = getNode( node ); // el=document.getElementById(id); // alert(el.style.toString());
  // try
  { if (!el.style.display || el.style.display==defview) el.style.display='none';
    else if (el.style.display=='none') el.style.display=defview;
  } // catch(e) { alert('exception:a2 '+e.toString()); }
}
...
function timePopulate( keyNode, locks ) { // alert(keyNode);
    ...
    var kyId = keyNode.getAttribute('id');
    var tpl = '_add_start_time'; // '_add_newtime';  var tpl = '_add_start_time';document.timeCnt
...
function validateTime ( node, locks ) {
  ...
  var el = getNode( node );  // alert('*** : '+node2string(node));
  var reasons='';
...
function csvStrInject( v ) {
  v = v.replace(new RegExp(";", "g"), ',');
  v = v.replace(new RegExp("[\r\n\f\v\t\b ]", "g"), ',');  //v = v.replace(new RegExp(",", "g"), ' ');
  v = v.replace(new RegExp(",{2,30}", "g"), ',');
  var vs = v.split(','); //alert( v );  alert( vs );
  var patrn = new RegExp("[0-1][0-9]|2[0-3]"+"[-.:]{1}"+"[0-5]\d", "g" );
  var tm, errMsg, vt = '', showTimes = [];
  var tpl = '_add_start_time';

  for ( var i=0; i<vs.length;i++ ) {
    if ( vs[i].length==5 ) {
      tm=vs[i].slice(0,2)+':'+vs[i].slice(3);
      errMsg = isTime(tm);
      if (!errMsg.length)  {
        showTimes[showTimes.length] = tm;
        var curTm = getNode(tpl+document.timeCnt);
        curTm.value = tm;
        timePopulate( curTm, ['btnTimeAdd'] ); // curTm.onchange();
      }
