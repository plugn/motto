...
/**  class: ThemePlayTE
   ...
   ! feature list:
     ...
     - простой парсинг списков с поддержкой разделителя(cfg->loop_delim) и выделения элементов ( cfg->loop_curr) :
         Loop($data, $tpf, $store_to="", $append=0, $implode=false, $wrap_empty=true)
   ...
   + new features:
     - конструкция проверки условий в шаблонах
     {{?[*]var}} | {{?![*]var}} ... [ {{?:}} ] ... {{?/}} | {{/?}}
...
/*========================== /RocketTE ====================================*/
  ...
  function TplRead( $tpl ) {
    ...
    if ( isset($this->recent_cache[$tpl]) )
      return $_aa.$this->recent_cache[$tpl].$_zz ;
  ...
  function Parse( $tpl, $store_to="", $append=0 ) {    // checkout!
    ...
    ob_start();
      $evalcode = "?>" . $content. "<?php "; // echo $evalcode;
      eval( $evalcode );
  ...
  function _Preparse($content) {
    ...
    // if..else.   typo correcting {{/?}} => {{?/}}
    $content =  str_replace(  $this->cfg->tpl_tag_open. "/".$this->cfg->tpl_construct_if. $this->cfg->tpl_tag_close,
                              $this->cfg->tpl_tag_open. $this->cfg->tpl_construct_ifend.  $this->cfg->tpl_tag_close,
                              $content );
    $content = str_replace( $this->cfg->tpl_tag_open.$this->cfg->tpl_construct_ifend. $this->cfg->tpl_tag_close, ' <?php } ?> ', $content );
    $content = str_replace( $this->cfg->tpl_tag_open.$this->cfg->tpl_construct_ifelse.$this->cfg->tpl_tag_close, ' <?php } else { ?> ', $content );

    $ptrn_ifnot = $this->cfg->tpl_tag_open.preg_quote($this->cfg->tpl_construct_if).'!(\*?)(.*?)'. $this->cfg->tpl_tag_close;
    $ptrn_if    = $this->cfg->tpl_tag_open.preg_quote($this->cfg->tpl_construct_if). '(\*?)(.*?)'. $this->cfg->tpl_tag_close;
    $content = preg_replace( '/'.$ptrn_ifnot.'/ims', ' <?php if (!$this->domain["\\2"] ) { ?> ', $content );
