...
class CmxGuide extends Guide {
  ...
  function Handle()  {
    ...
    if ($uri_data=='' || $uri_data=='waproot') $uri_data = 'waproot'; // project root empty request patch
    // cache preface routine. если есть в кэше отдать контент и завершение
    $this->uri_hash = base64_encode( str_replace('page=index.php&page=', '', $uri_data) );
    // $this->dbgMsg ( ' $this->uri_hash : ',  $this->uri_hash , $pre=1, $htmlqt=0 );
    header("Content-type: text/vnd.wap.wml; charset=utf-8");

    /* if ( $this->tpl_caching && txCache::CachePreface( $this->proj_cache_dir.$this->uri_hash )) { */
    if ( $this->tpl_caching && txCache::CacheCheck( $this->proj_cache_dir.$this->uri_hash ) ) {
        ob_start();
            $this->dbgMsg ( ' WAP. ['.date("H-i-s").'] cache : '.$this->uri_hash, $this->uri, $pre=0, $htmlqt=0 );
    ...
    if ($this->tpl->Get("HTML:Html"))
      $this->Output( iconv("cp1251", "UTF-8", $this->tpl->Get("HTML:Html")) ); //  echo $this->tpl->Get("HTML:Html");var_export($this->tpl->domain);
